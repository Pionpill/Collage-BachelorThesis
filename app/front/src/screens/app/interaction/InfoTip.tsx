...
const InfoTip: React.FC<{ markerInfoShort: MarkerInfoShort }> = ({
  ...
  const showTip = useSelector((root: RootState) => root.interaction.showTip);
      ...
      <FlexBox gap={2} m={2} justify="space-between" fullWidth>
        ...
        <FlexBox gap={1}>
          <IconButton
            ...
            disableRipple
            onClick={() => dispatch(changeInteractionInfoCard(true))}
          >
          ...
          <IconButton
            sx={{ ml: "auto", bgcolor: "theme" }}
            disableRipple
            onClick={() => dispatch(changeInteractionTipCard(false))}
          >
